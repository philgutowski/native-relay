...
**Why validate said nothing, and what it does now.** Until issue 10, `manifest.validate` checked the Manifest and, with `check_repo`, the repository's shape, and had no reference to `branch_exists`, `task_branch_for`, or `preflight`. Preflight is a per Task, launch time check, and validate never composed a Task branch name, so a Manifest naming a Task whose branch was already in flight was valid by every rule validate applied. Now `validate` composes each listed, non excluded Task's branch from the Manifest's resolved prefix, reads the local branch list with the same `branch_exists` preflight uses, reads origin with `gitread.remote_heads`, and warns once per Task with a hit, naming the branch, where it was found, and the repair below. It is a warning and not an error, and the exit code stays 0, so exit 0 still does not mean ready to launch while a warning stands. A branch found only on origin is warned about too, although preflight would not refuse it, because the fresh Task process would not know the earlier work is there. Only the `validate` verb pays for the origin read: `run`, `dispatch`, and the pair verbs pass `check_branches=False`, since they discard warnings. The warning cannot tell a stranded branch from the current run's own, so a validate during a live run or after a halt will name that Task's branch too; leave it alone and read `status`.
