...
def _pending_checks(entries, run_status, halt_task, halt_class, state_dir, card_audit=None):
    ...
    for entry in entries:
        ...
        for finding in entry["findings"]:
            ...
            elif finding["class"] == contracts.REVIEW_FAILED:
                checks.append({"kind": "review_failed", "task": task_id,
                               "text": "%s produced rejected evidence for %s: %s. Review the "
                                       "diff by hand." % (task_id,
                                                         finding.get("review") or "the review step",
                                                         finding.get("reason") or "unknown reason")})
